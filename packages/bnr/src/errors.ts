...
export interface BnrError {
  readonly kind: BnrErrorKind
  readonly message: string
  readonly status?: number
}

/** Thrown by the transport on a non-2xx, so the client can tell it from a dropped connection. */
export class BnrHttpError extends Error {
  constructor(
    readonly status: number,
    readonly statusText: string,
  ) {
    super(`BNR replied ${status} ${statusText}`)
    this.name = 'BnrHttpError'
  }
}
